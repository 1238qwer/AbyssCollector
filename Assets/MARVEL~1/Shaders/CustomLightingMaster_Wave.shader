...
//----------------------------------------------
Shader "Kirnu/Marvelous/CustomLightingMaster_Wave" {
		...
		Tags { "LIGHTMODE"="ForwardBase" "QUEUE"="Geometry" "RenderType"="Opaque" }
			CGPROGRAM
				...
				CL_OUT_WPOS vert(CL_IN v) {
					...
					float2 uv_MainTex = TRANSFORM_TEX (v.texcoord, _MainTex);
					v.vertex.x += (sin((v.vertex.x / _WaveLength) + (_WaveFrequency * _Time)) * _WaveMagnitude) * uv_MainTex.y;
